...
- **The optional embedding-retrieval addition's similarity threshold has a genuinely thin safety margin.** `EMBEDDING_SIM_THRESHOLD = 0.70` was calibrated against this session's real matcher bugs (see `agent/embedding_match.py`), but a live production query for "chickpea flour" — the case this addition was built to solve — scored 0.6997 against the catalog's actual "Gram Flour" product: **0.0003 below the cutoff**, purely from the LLM's real wording differing slightly from the hand-picked calibration phrase. It fell back to the existing DIY-substitute cascade instead (a reasonable, honestly-labeled result on its own), rather than a direct match. Lowering the threshold to catch this case would also narrow the margin against the known false positives it was calibrated to reject (caviar/Koi-fish scores 0.66), which is the wrong trade — so the threshold is left as-is. Concrete evidence, not just a caveat, that Layer 1's embedding tier is measurably less robust than Layer 2's deterministic logic, and a fair illustration of why this whole addition stays optional. (Full catalog is backfilled — 2,807/2,807 products embedded across 3 rotating API keys.)
