...
The differentiator: a function-calling agent, not a Q&A chatbot. Tell it what you're cooking ("morog polao for 6", "biryani under 1500 taka") and it interprets the request, matches ingredients against the real catalog, and fills your cart — substituting or honestly skipping whatever's out of stock. It handles plain, non-recipe requests the same way — "add ketchup," "remove the rice," "empty my cart" — not just full dish requests; see the `POST /chat` bullet below for the exact set of intents.
...
- **`POST /chat`** (`backend/routers/chat.py`) connects the two: runs the agent, then merges whatever it matched into the customer's real cart through the same code path the regular "add to cart" endpoint uses. Ten intents in total: `cook_dish`/`budget_dish` (the recipe flow above), `add_items` (a plain single/multi-item request like "add ketchup," no dish involved), `remove_items`/`clear_cart`/`keep_only_items` (act directly on the customer's existing cart, no catalog lookup needed), `modify_dish` (swap one ingredient for another mid-conversation, e.g. "make it beef" after ordering chicken biryani), `product_question`/`ingredient_question` (the two question types above), and `other` as an honest catch-all.
...
- **Backend cold starts on Render's free tier** after ~15 minutes of inactivity — the first request afterward can take up to ~50 seconds. The frontend polls `/health` and shows a "waking up" banner rather than hiding the delay. Two independent keep-alive pingers hit `/health`: [UptimeRobot](https://uptimerobot.com) (the primary one, checking every 5 minutes) and a scheduled GitHub Actions workflow (`.github/workflows/keep-alive.yml`, configured for every 10 minutes) as a backup. UptimeRobot became the primary one specifically because the GitHub Actions cron turned out not to be reliable enough alone — checked against the actual run history, not assumed: configured for `*/10 * * * *`, but real gaps between runs ranged from ~52 minutes to over 3.5 hours, well past Render's ~15-minute sleep window, meaning cold starts were likely still happening regularly even with the workflow active. This is a known, documented GitHub Actions limitation (scheduled workflows are "best effort" and can be delayed significantly, especially on lower-activity repos), not a bug in the workflow file itself. GitHub Actions is kept as a backup regardless, since it doesn't depend on a third-party service staying up — but UptimeRobot, running on its own dedicated infrastructure, is what actually keeps the interval tight in practice; it also doubles as real downtime alerting, and its own 60-day-inactivity auto-disable risk (a real GitHub Actions limitation) is a secondary reason to not rely on the workflow alone. `/health` accepts both `GET` and `HEAD` (`backend/main.py`) specifically because UptimeRobot, like most uptime monitors, checks via `HEAD` by default — a plain `@app.get`-only route 405s on that and reads as "down" even when the backend is perfectly healthy, a real false alarm this project hit and fixed live.
